...
function computeCompletion(
  ...
  if (essaysTotal === 0 && recsTotal === 0) return 0;
  const essayScore = essaysTotal > 0 ? (essaysDone / essaysTotal) * criteria.essayWeight : 0;
  const recScore = recsTotal > 0 ? (recsDone / recsTotal) * criteria.recWeight : 0;
...
serve(async (req) => {
  ...
  try {
    const { schoolId, criteria } = await req.json() as {
      schoolId: string;
      criteria: AtRiskCriteria;
    };

    if (!schoolId || !criteria) {
      return new Response(
        JSON.stringify({ error: "schoolId and criteria are required" }),
    ...
    if (studentIds.length === 0) {
      return new Response(
        JSON.stringify({ stats: emptyStats, analytics: emptyAnalytics }),
    ...
    const [essaysRes, appsRes, recsRes, assignmentsRes] = await Promise.all([
      ...
      supabase
        .from("applications")
        .select("student_id, school_name, deadline_date, status")
        .in("student_id", studentIds),
      supabase
    ...
    const profileMap = new Map(allMembers.map(m => [m.user_id, m]));

    // ── Per-student essay / rec maps ──────────────────────────
    ...
    const urgentSet = new Set(urgentApps.map(a => a.student_id));

    const scoreOf = (sid: string): number => {
    ...
    const riskOf = (sid: string) => classifyRisk(scoreOf(sid), urgentSet.has(sid), criteria);
    ...
    for (const sid of studentIds) {
      if (riskOf(sid) === "at-risk") atRiskCount++;
    }
    ...
    const stats = {
      ...
      totalRecLetters: allRecs.length,
      atRiskCount,
      essaysPending,
      urgentApplications: urgentApps.length,
    };
    ...
    const topCounselors = counselorIds
      .map(cid => {
        const sids = cStudentMap.get(cid) ?? [];
        const scores = sids.map(scoreOf);
        const avg = scores.length > 0 ? Math.round(scores.reduce((a, b) => a + b, 0) / scores.length) : 0;
    ...
    if (essaysPending > 0)
      insights.push(`${essaysPending} essay${essaysPending > 1 ? "s are" : " is"} awaiting counselor review.`);
